...
|---|---|---|
| **Tenant 1** | 🪑 Luxe Haven Furniture | `join average-grandfather` |
| **Tenant 2** | 🚗 PrimeAuto Care | `join shut-since` |
...
**Try with Tenant 1 — Luxe Haven Furniture:**
...
**Try with Tenant 2 — PrimeAuto Care:**
...
|---|---|---|
| **Tenant A** | Luxe Haven Furniture | Sells luxury furniture, sends product catalog PDFs, showroom images |
| **Tenant B** | PrimeAuto Care | Automotive service center, schedules appointments, sends invoices, analyzes car damage photos |
...
The assignment specifies Meta WhatsApp Cloud API. The full Meta integration is **completely implemented** in `app/whatsapp/client.py` (read receipts, typing indicators, text, image, document). However, during development our test WhatsApp Business Account was **restricted by Meta**, preventing inbound message testing without submitting business verification documents.

We faced a choice:

> *"Would you like me to submit the repository in its current state with the Meta integration ready to work using your verified credentials? Or migrate to Twilio for a fully working live demo?"*

We chose to **migrate to Twilio** to deliver a working end-to-end demo rather than a partial implementation that requires external credentials to verify.
...
**Example trigger:** *"Can you send me your product catalog?"* → AI calls `send_catalog_pdf("catalog", "Here is our 2026 Luxe Haven catalog!")`
